 ...
 * the paths can start or end at any node but all paths must
...
public class CountAllPathSum {
     ...
     * Space: O(h) ~ [logn, n]
     *
     * @param currentNode the curr tree node in stack
     * @param S the target sum
     * @param currentPath the nodes that we need to consider
     * @return # of paths found whose vals sum to S
     */
    private static int countPathsRecursive(TreeNode currentNode, int S, List<Integer> currentPath) {
        ...
        ListIterator<Integer> pathIterator = currentPath.listIterator(currentPath.size()); // size: O(h)
